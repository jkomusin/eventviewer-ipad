...
- (void)swapPanelLabels:(NSInteger)draggingIndex and:(NSInteger)otherIndex whileDragging:(BOOL)dragging
{
    ...
    CGRect otherLabelF = otherLabel.frame;
	CGRect labelF = otherLabelF;
    labelF.origin.x = panelX;
    otherLabel.frame = labelF;
	
	if (!dragging)
	{
		draggingLabel.frame = otherLabelF;
	}
...
{        
    ...
    for (int i = 0; i < [_panelLabelArray count]; i++)
    {
        if (draggingLabel == [_panelLabelArray objectAtIndex:i])
        {
            ...
            draggingLabelType = UIObjectPanel;
    ...
    if (draggingLabelType == -1)
    {
        for (int i = 0; i < [_stackLabelArray count]; i++)
        {
            if (draggingLabel == [_stackLabelArray objectAtIndex:i])
            {
                ...
                draggingLabelType = UIObjectStack;
        ...
        {   
            ...
            for (int j = 0; j < [bandArray count]; j++)
            {
                if (draggingLabel == [bandArray objectAtIndex:j])
                {
                    ...
                    draggingLabelType = UIObjectBand;
    ...
    if (_draggingType != draggingLabelType 
        ||
        (draggingLabelType == UIObjectBand && [_draggingLabels count] > 0 &&
    ...
    for (l = 0; l < [mutaDraggingLabels count]; l++)
    {
        ...
        if (((draggingLabelType == UIObjectBand || draggingLabelType == UIObjectStack) && draggingLabel && (currentL.frame.origin.y > draggingLabel.frame.origin.y))
            ||
            (draggingLabelType == UIObjectPanel && draggingLabel && (currentL.frame.origin.x > draggingLabel.frame.origin.x))) 
        {
            break;
        }
    }
...
//    [CATransaction setDisableActions: YES];
    ...
    if (draggingLabelType == UIObjectPanel)
    {
    ...
    if ((yDiff < 0 || xDiff < 0) && draggingLabelIndex > 0)
    {
        ...
        else if (draggingLabelType == UIObjectBand)
        ...
        (!reorderUp && (swappingLab.center.y < draggingLabel.center.y || swappingLab.center.x < draggingLabel.center.x))))
    {
        if (draggingLabelType == UIObjectPanel)
        {
            ...
            [_dataDelegate swapPanelData:panelIndex withPanel:swappingPanelIndex];
        }
        else if (draggingLabelType == UIObjectStack)
        {
            [self swapStackLabels:stackIndex and:swappingStackIndex whileDragging:YES];
            for (PanelZoomView *p in _panelZoomViews)
                [p.panelDrawView reorderStack:stackIndex withNewIndex:swappingStackIndex];
            // Inform data delegate that reordering is needed
            [_dataDelegate swapStackData:stackIndex withStack:swappingStackIndex];
        }
        else if (draggingLabelType == UIObjectBand)
        {
            ...
            // Inform data delegate that reordering is needed
            [_dataDelegate swapBandData:bandIndex withBand:swappingBandIndex];
        }
   ...
   else   
    {
        ...
        {
            if ((draggingLabelType == UIObjectPanel && ([[a objectAtIndex:4] intValue] == newIndex)) ||
                (draggingLabelType == UIObjectStack && ([[a objectAtIndex:3] intValue] == newIndex)) ||
                (draggingLabelType == UIObjectBand && ([[a objectAtIndex:2] intValue] == newIndex)))
            {
                beingDragged = YES;
                break;
            }
        ...
        // Reorder
        if (draggingLabelType == UIObjectPanel && (newIndex != panelIndex) && (newIndex >= 0) && !beingDragged)
        {
            ...
            [self swapPanelLabels:panelIndex and:newIndex whileDragging:YES];
            // Inform data delegate
            [_dataDelegate swapPanelData:panelIndex withPanel:newIndex];
        ...
        else if (draggingLabelType == UIObjectStack && (newIndex != stackIndex) && (newIndex >= 0) && !beingDragged)
        {
            // Check for legal indices
            ...
            [self swapStackLabels:stackIndex and:newIndex whileDragging:YES];
            // Inform data delegate that reordering is needed
            [_dataDelegate swapStackData:stackIndex withStack:newIndex];
        ...
        else if (draggingLabelType == UIObjectBand && (newIndex != bandIndex) && (newIndex >= 0) && !beingDragged)
        {                
            // Check for legal indices
            ...
            // Inform data delegate that reordering is needed
            [_dataDelegate swapBandData:bandIndex withBand:newIndex];
